...
class DatabaseManager:
    ...
    def get_daily_revenue(self, date_str: str) -> float:
        ...
        try:
            ...
            cursor.execute("""
                SELECT COALESCE(SUM(order_total_amount), 0) as daily_revenue
                FROM Orders 
                WHERE DATE(order_date) = ? AND order_status = '已完成'
            """, (date_str,))
    ...
    def get_monthly_revenue(self, year: int, month: int) -> float:
        ...
        try:
            ...
            cursor.execute("""
                SELECT COALESCE(SUM(order_total_amount), 0) as monthly_revenue
                FROM Orders 
            ...
            result = cursor.fetchone()
            return float(result['monthly_revenue']) if result else 0.0
            
        except Exception as e:
            print(f"❌ 获取月收入失败: {e}")
            return 0.0
    
    def get_daily_orders_count(self, date_str: str) -> int:
        """获取指定日期的订单数量"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            result = cursor.fetchone()
            return int(result['order_count']) if result else 0
            
        except Exception as e:
            print(f"❌ 获取日订单数失败: {e}")
            return 0
    
    def get_product_sales_stats(self, days: int = 30) -> List[Dict]:
        """获取产品销售统计（最近N天）"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
