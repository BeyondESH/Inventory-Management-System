...
class DataManager:
    ...
    def get_financial_records(self) -> List[Dict]:
        """获取财务记录"""
        return self.financial_records.copy()    def load_financial_records(self) -> List[Dict]:
        """加载财务记录（JSON模式）"""
        try:
    ...
    def save_financial_records(self):
        ...
        try:
            ...
            with open(finance_file, 'w', encoding='utf-8') as f:
                json.dump(self.financial_records, f, ensure_ascii=False, indent=2, cls=CustomJSONEncoder)
        except Exception as e:
            print(f"保存财务数据失败: {e}")
        ...
        try:
            ...
            for ingredient_name in ingredients:
                ...
                if not inventory_item:
                    print(f"⚠️ 库存中未找到食材: {ingredient_name}")
                    continue
            ...
            # 如果有库存不足的情况，记录警告但继续执行
            if insufficient_items:
                for item in insufficient_items:
                    print(f"⚠️ 库存不足: {item['name']} (需要: {item['required']:.2f}, 可用: {item['available']:.2f})")
